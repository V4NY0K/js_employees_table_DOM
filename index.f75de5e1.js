...
tbody.querySelectorAll("tr").forEach((tr)=>{
    if (!tr.dataset.id) tr.dataset.id = String(nextId++);
});
let activeRow = null;
// selecting logic
...
thead.addEventListener("click", (e)=>{
    ...
    if (!thKey) return;
    if (thKey === sortState.key) sortState.dir = sortState.dir === "asc" ? "desc" : "asc";
    else {
        sortState.key = thKey;
        sortState.dir = "asc";
    }
    const rows = getDataFromDom();
    const sorted = rows.slice().sort((a, b)=>compare(a, b, sortState.key, sortState.dir));
...
function compare(a, b, key, dir = "asc") {
    ...
    let result;
    if (!Number.isNaN(aNum) && !Number.isNaN(bNum)) result = aNum - bNum;
    else result = String(va).localeCompare(String(vb), undefined, {
        numeric: true
    });
...
form.addEventListener("submit", (e)=>{
    ...
    if (workerName.length < 4 || Number.isNaN(age) || age < 18 || age > 90 || !Number.isFinite(salary) || position.trim().length < 2 || !regex.test(position.trim())) {
    ...
    headers.forEach((value, i)=>{
        ...
        if (i === headers.length - 1) td.textContent = currencyFormatter.format(value);
...
function showNotification(type) {
    const notify = document.querySelector('[data-qa="notification"]') || document.createElement("div");
    ...
    if (!document.body.contains(notify)) document.body.appendChild(notify);
...
tbody.addEventListener("dblclick", (e)=>{
    ...
    if (existing && existing !== td.querySelector(".cell-input")) existing.blur();
    let textNode;
    for (const node of td.childNodes)if (node.nodeType === Node.TEXT_NODE && node.textContent.trim()) {
        textNode = node;
        break;
    }
